...
Since `id` is meaningless to house price, it will be removed. There are several variables that should be converted to categorical types:
...
* After removing `r diff` from the model, both new stepwise models still maintain the same r squared as the final log model. 
* Both model remove `r diff`, indicating that these features are less important than others.

## Conclusion & Improvement

1. The regression model improves a lot when some features are converted into categorical types.
